...
public class CoinChangeCountMaxCoinsToGetAmount {
    public static void main(String[] args) {
        int[] denominations = new int[]{2, 3, 5};
        int amount = 8;
        System.out.println("coins values:" + Arrays.toString(denominations));
        System.out.println("amount:" + amount);
        CoinChangeCountMaxCoinsToGetAmount cc = new CoinChangeCountMaxCoinsToGetAmount();
    ...
    private int coinChangeTopDownDP(Integer[][] dp, int[] coins, int amount, int currentIndex) {
        ...
        dp[currentIndex][amount] = Math.max(count1, count2);
        return dp[currentIndex][amount];
    }
    ...
    private int coinChangeUsingBottomUpDP(int[] coins, int amount) {
        ...
        for (int i = 0; i < n; i++) {
            for (int s = 1; s <= amount; s++) {
                ...
                dp[i][s] = Math.max(count1, count2);
            }
        }
        // the bottom-right corner will have our answer.
        return (dp[n - 1][amount] == Integer.MIN_VALUE ? -1 : dp[n - 1][amount]);
    }
}
