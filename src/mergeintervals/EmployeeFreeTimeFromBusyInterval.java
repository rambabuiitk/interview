...
public class EmployeeFreeTimeFromBusyInterval {

    public static void main(String[] args) {
        List<List<Interval>> input = new ArrayList<>();
        input.add(new ArrayList<>(Arrays.asList(
                new Interval(1, 3),
                new Interval(5, 6)))
        );
        input.add(new ArrayList<>(Arrays.asList(
    ...
    private static List<Interval> findEmployeeFreeTime(final List<List<Interval>> schedule) {
        ...
        while (!minHeap.isEmpty()) {
            ...
            if (employeeSchedule.size() > queueTop.intervalIndex + 1) {
                // add the next interval, increment the intervalIndex 
